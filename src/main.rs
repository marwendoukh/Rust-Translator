...
\n Translate to :");
    ...
    client.get(&link)
		    ...
		    // put the result to the string declared above
                    .read_to_string(&mut translation_result)
                    .unwrap();

    // format the result as JSON
    let data = rustc_serialize::json::Json::from_str(&translation_result).unwrap();

    // obj will contain the JSON object
    let obj = data.as_object().unwrap();
    //from the JSON object created , get only the result of translation
    let translated_word = obj.get("text").unwrap();
    let translated_from_to = obj.get("lang").unwrap();
    // display it
    println!("translation is : {} ", translated_word[0]);
    println!("translated from-to  : {} ", translated_from_to);
}
